    ...
    public abstract class PadraoDAO<T> where T : PadraoViewModel
    {
        ...
        public virtual void Update(T model)
        {
            if(model is SensorViewModel)
            {
                ...
                sensor = disp.UpdateSensor(model as SensorViewModel);

                (model as SensorViewModel).Temperatura = sensor.Temperatura;
                (model as SensorViewModel).Umidade = sensor.Umidade;
                (model as SensorViewModel).DataAtualizacao = DateTime.Now.ToString();

                HelperDAO.ExecutaProc("spUpdate_Sensor", CriaParametros(model));
            }
            else
            { 
